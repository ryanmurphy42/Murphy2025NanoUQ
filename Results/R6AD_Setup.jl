#=
    R6AD - 633 nm PMA core-shell particles with THP-1 cells
...
filepath_load_ini = pwd() * filepathseparatorini * "Data" * filepathseparatorini * "P_633nm_thp1" * filepathseparatorini;
dataini = read(Inifile(), filepath_load_ini * "leo_633nm_coreshell_thp1.ini")
...
filepath_load = pwd() * filepathseparator * "Data" * filepathseparator * "P_633nm_thp1" * filepathseparator;
datafiles = ["THP-1 - 519 nm CS 1 hr.fcs.csv",
            "THP-1 - 519 nm CS 2 hr.fcs.csv",
            "THP-1 - 519 nm CS 4 hr.fcs.csv",
            "THP-1 - 519 nm CS 8 hr.fcs.csv",
            "THP-1 - 519 nm CS 16 hr.fcs.csv",
            "THP-1 - 519 nm CS 24 hr.fcs.csv"];
lendatafiles = length(datafiles);
datafiles2 = ["THP-1 - 519 nm CS 1 hr (1).fcs.csv",
            "THP-1 - 519 nm CS 2 hr (1).fcs.csv",
            "THP-1 - 519 nm CS 4 hr (1).fcs.csv",
            "THP-1 - 519 nm CS 8 hr (1).fcs.csv",
            "THP-1 - 519 nm CS 16 hr (1).fcs.csv",
            "THP-1 - 519 nm CS 24 hr (1).fcs.csv"];
...
data_particleonly = CSV.read([filepath_load * "519 nm CS (AF647N3).fcs.csv"],DataFrame);
...
struct MyMetricSumAD <: Distances.UnionMetric end

@inline function eval_op_AD(::MyMetricSumAD, b,a) 
  # a - observed data
  # b - simulated data
   
    sumADdistance = 0;
    for i=1:size(b,1) # for each timepoint
        n =length(b[i])  # number of data points
        b_i_sort = sort(b[i]);

        Fobs = [min(1-1e-9, max(1e-9,a_ecdf[i](b_i_sort[k]))) for k=1:n];
    
        ADdistance = -n - sum([((2*k-1)/n)*( log(Fobs[k]) + log(1 - Fobs[n+1-k])) for k=1:n]);
      sumADdistance = sumADdistance + ADdistance;
    end

    return sumADdistance
end
@eval @inline (dist::MyMetricSumAD)(a::AbstractArray, b::AbstractArray) = eval_op_AD(dist, a, b)
const mymetricsumAD = MyMetricSumAD()

# ABC distance 
function makediscrepancy(X)
    dist = mymetricsumAD(X,zeros(1))
    return dist
end
