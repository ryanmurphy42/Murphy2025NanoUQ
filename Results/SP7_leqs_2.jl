#=
    SP7_lesq - Synthetic data with low r (relative to K) - KNOWN SYNTHETIC CONTROL DATA
...
scatter!(fig_raw_median,[0;Traw],[0;median_data_syn],color=:black,markersize=10,xticks=[0,12,24],yticks=[0,10,20,30],ylims=(-0.5,30.5))
...
Tplot = LinRange(0,25,251);
plot!(fig_raw_median_leastsquares,Tplot,[solve_ode_model_independent(Tplot[i]*3600,xopt,θfixed) for i=1:length(Tplot)],color=:red,lw=3,xticks=[0,12,24],ylims=(-0.1,3),ls=:dash)
...
@save "Results/Files_syn/Lesq_SP7.jld2" Tplot xopt θfixed median_data_syn