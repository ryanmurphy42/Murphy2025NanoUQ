#=
    R1AD_lesq - 150nm PMA core-shell particles with THP-1 cells.
...
# particle only data
fig_data_particleonly = plot(layout=grid(1,1),
                            xlab="S [AU]",
                            legend=false,
                            size = (50*mm_to_pts_scaling,33*mm_to_pts_scaling),
                            titlefontsize=10,
                            guidefontsize=10,
                            tickfontsize=10,
                            legendfontsize=10)
histogram!(fig_data_particleonly,data_particleonly[:,"638 Red(Peak)"],xlim=[0,1000],ylim=[0,0.02],label="Raw",normalize=:pdf,color=:gray80, bins = 0:20:1000,lw=0.5)
vline!(fig_data_particleonly,[median(data_particleonly[:,"638 Red(Peak)"])],label="Median",color=:red,lw=2,xticks=[0,500,1000],yticks=[0,0.01,0.02],ls=:dash)
display(fig_data_particleonly)
savefig(fig_data_particleonly,filepath_save * "fig_data_particleonly" * ".pdf")

# cell only data
fig_data_cellonly = plot(layout=grid(1,1),
                            xlab="S [AU]",
                            legend=false,
                            size = (50*mm_to_pts_scaling,33*mm_to_pts_scaling),
                            titlefontsize=10,
                            ...
                            tickfontsize=10,
                            legendfontsize=10)
histogram!(fig_data_cellonly,pmt_correctionfactor(voltage_cellonly,voltage_particleonly).*data_cellonly[:,"638 Red(Peak)"],xlim=[0,1000],ylim=[0,0.01],label="Raw",normalize=:pdf,color=:gray80, bins = 0:20:1000,lw=0.5)
vline!(fig_data_cellonly,[median(pmt_correctionfactor(voltage_cellonly,voltage_particleonly).*data_cellonly[:,"638 Red(Peak)"])],label="Median",color=:red,lw=2,xticks=[0,500,1000],yticks=[0,0.01],ls=:dash)
...
for i=1:lendatafiles
    fig_data = plot(layout=grid(1,1),                            
                    ...
                    tickfontsize=10,
                    legendfontsize=10)
    histogram!(fig_data,pmt_correctionfactor(voltage_data[i],voltage_particleonly).*data[i][:,"638 Red(Peak)"],xlim=[0,1000],ylim=[0,0.01],label="Raw",normalize=:pdf,color=:gray80, bins = 0:20:1000,lw=0.5)
    vline!(fig_data,[median(pmt_correctionfactor(voltage_data[i],voltage_particleonly).*data[i][:,"638 Red(Peak)"])],label="Median",color=:red,lw=2,xticks=([0,500,1000], ["0", "500", "1000"]),yticks=([0,0.01], ["0.00","0.01"]),ls=:dash)
    display(fig_data)
...
scatter!(fig_raw_median,[0;Traw],[0;median_data_syn],color=:black,markersize=10,xticks=[0,12,24],yticks=[0,5,10,15],ylims=(-0.5,15.5))
...
Tplot = LinRange(0,25,251);
plot!(fig_raw_median_leastsquares,Tplot,[solve_ode_model_independent(Tplot[i]*3600,xopt,θfixed) for i=1:length(Tplot)],color=:red,lw=3,xticks=[0,12,24],ylims=(-0.5,15.5),ls=:dash)
...
@save "Results/Files_real/R1_Lesq.jld2" Tplot xopt θfixed median_data_syn
