...
# #################################################
# # 5 - ABC Distance metric - Cramer von Mises
# #################################################

a_ecdf = [ecdf(X[i]) for i=1:length(T)];  # Empirical cdf of data

struct MyMetricSumCV <: Distances.UnionMetric end

@inline function eval_op_CV(::MyMetricSumCV, b,a) 
  # a - observed data
  # b - simulated data
    sumCVdistance = 0;
    for i=1:size(b,1) # for each timepoint
        ...
        CVdistance = (1/(12*n)) + sum([(((2*k-1)/(2*n)) - a_ecdf[i](b_i_sort[k]))^2 for k=1:n])
        sumCVdistance = sumCVdistance + CVdistance;
    end
    return sumCVdistance
    
end
@eval @inline (dist::MyMetricSumCV)(a::AbstractArray, b::AbstractArray) = eval_op_CV(dist, a, b)
const mymetricsumCV = MyMetricSumCV()

# ABC distance 
function makediscrepancy(X)
    dist = mymetricsumCV(X,zeros(1))
    return dist
end
