#=
    SQ07 - Generate synthetic data with low r (relative to K) - HOMOGENEOUS r,K - SYNTHETIC CONTROL DATA
=#
...
CSV.write(filepath_save * "data_syn_SQ07_CELLONLY500V" * ".csv", df_cellonly) # Export to csv
...
# Load CSV
data_cellonly = CSV.read([filepath_save * "data_syn_SQ07_CELLONLY500V" * ".csv"],DataFrame);
...
filepath_save = pwd() * "/Data/Synthetic/"
isdir(filepath_save) || mkdir(filepath_save); # make folder to save figures if doesnt already exist
CSV.write(filepath_save * "data_syn_SQ07_PARTICLEONLY600V" * ".csv", df_particleonly) # Export to csv
...
# Load CSV
data_particleonly = CSV.read([filepath_save * "data_syn_SQ07_PARTICLEONLY600V" * ".csv"],DataFrame);
...
CSV.write(filepath_save * "data_cellonly_syn_SQ07.csv", data_cellonly_syn) # Export to csv
...
CSV.write(filepath_save * "data_particleonly_syn_SQ07.csv", data_particleonly_syn) # Export to csv
...
for i=1:length(T)
    ...
    CSV.write(filepath_save * "data_syn_SQ07_" * string(i) * ".csv", data_syn) # Export to csv
end
